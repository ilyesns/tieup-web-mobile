...
import { createUser, mayBeCreateUser } from "../../apis/user_api";
import { useNavigate } from "react-router-dom";

export const PopUpSignUp = React.memo(
  ({ isVisible, onClose, onRequestClose, modelRef }) => {
    ...
    const handleFacebookSignIn = async () => {
      try {
        const user = await signInWithFacebook().catch((e) => {
          setErrorMessage(e);
        });
        onRequestClose(true);
      } catch (e) {
    ...
    const handleGoogleSignIn = async () => {
      try {
        ...
        let firstName = splitFullName(
          credentialUser.user.displayName
        ).firstName;
    ...
    return (
      <div className="fixed inset-0 z-50 bg-black bg-opacity-30 backdrop-blur-sm flex justify-center items-center">
        <div ref={modelRef} className="flex flex-col">
          <button
            onClick={() => {
              onClose();
            }}
          ...
          <div className="w-full md:w-[900px]  bg-white rounded-md  p-7 overflow-y-auto  ">
            <div className="flex flex-row  justify-center ">
              <div className=" flex  w-full gap-3 p-5 items-center rounded-md">
                <div className="w-1/2  ">
